...
try { 
  const swaggerModule = await import('swagger-parser');
  SwaggerParser = swaggerModule.default || swaggerModule;
} catch (_) { 
  SwaggerParser = null; 
}
  ...
  try {
    ...
    for (const path in api.paths) {
      for (const method in api.paths[path]) {
        ...
        // Fallback: extract path parameters from path template if not already defined
        ...
        if (pathParamMatches) {
          for (const match of pathParamMatches) {
            ...
            if (!pathParams.includes(paramName)) {
              ...
              inputSchema.properties[paramName] = { type: 'string', description: `Path parameter: ${paramName}` };
